  ...
  async beforeCreate(event) {
    ...
    if (!data.DeliveryOrderNo) {
      ...
      const [entries, count] = await strapi.db
        .query(
          "api::sales-transaction-delivery-order.sales-transaction-delivery-order"
        )
        .findWithCount({
          select: ["DeliveryOrderNo", "createdAt"],
          where: { createdAt: { $gte: thisMonth } },
        });

      const uniqueNo = noFormatter.default.StandardFormat(
        constants.TransactionNoInitialEnum.DeliveryOrder,
        count
      );
      ...
      const [duplicateEntries, duplicateCount] = await strapi.db
        ...
        .findWithCount({
          select: ["DeliveryOrderNo", "createdAt"],
          where: { DeliveryOrderNo: { $contains: uniqueNo } },
        });

      if (duplicateCount === 0) {
        data.DeliveryOrderNo = uniqueNo;
      } else {
        data.DeliveryOrderNo = `${uniqueNo}/${duplicateCount + 1}`;
      }
    }
  },
};
